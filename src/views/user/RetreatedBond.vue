<template>
  ...
  <div class="ret-bond-list-page">
    
    <div class="table-filter">
      <div class="filter-item">
        <label for="created-time">退款时间：</label>
      ...
      <div class="filter-item">
        <label for="ticket-type">退款方式：</label>
        <el-select name="ticket-type" v-model="ticketType" size="small" filterable clearable placeholder="退款方式" class="filter-box">
          <el-option
            v-for="item in refundMethod"
            :key="item.number"
            :label="item.name"
            :value="item.number">
          </el-option>
    ...
    <el-table :data="list" highlight-current-row border :stripe="true" v-loading="listLoading" style="width: 100%;" class="filter-box" :header-row-style="{backgroundColor: '#f6f6f6'}" @sort-change="sortTable">
      ...
      <el-table-column  align="center" label="退款方式">
        <template slot-scope="scope">
          <span>{{ scope.row.user_name }}</span>
        </template>
      </el-table-column>
