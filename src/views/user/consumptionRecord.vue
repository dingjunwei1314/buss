<template>
  ...
  <div class="con-re">
    ...
    <el-table :data="list" highlight-current-row border :stripe="true" v-loading="listLoading" style="width: 100%;" class="filter-box" :header-row-style="{backgroundColor: '#f6f6f6'}" @sort-change="sortTable">
      ...
      <el-table-column align="center" label="消费金额（元）" width="120" sortable="custom" prop="stype">
        <template slot-scope="scope">
          <span>{{ scope.row.stype }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="在线支付金额（元）" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.title }}</span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="消费实充余额（元）" width="200">
        <template slot-scope="scope">
          <span v-html="scope.row.effective_type_time"></span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="消费返现余额（元）" width="200">
        <template slot-scope="scope">
          <span v-html="scope.row.effective_type_time"></span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="消费停车费余额（元）" width="200">
        <template slot-scope="scope">
          <span v-html="scope.row.effective_type_time"></span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="透支金额（元）" width="200">
        <template slot-scope="scope">
          <span v-html="scope.row.effective_type_time"></span>
        </template>
      </el-table-column>
      ...
      <el-table-column  align="center" label="消费时间" min-width="260">
        <template slot-scope="scope">
