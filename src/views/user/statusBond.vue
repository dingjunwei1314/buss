<template>
  ...
  <div class="status-bond-list-page">
    
    <div class="table-filter">
      <div class="filter-item">
        <label for="created-time">申请时间：</label>
      ...
      <div class="filter-item">
        <label for="created-time">处理时间：</label>
        <el-date-picker name="created-time" v-model="createdTime" type="daterange" size="small" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
      </div>  

      <div class="filter-item">
        <label for="ticket-type">退款状态：</label>
        <el-select name="ticket-type" v-model="ticketType" size="small" filterable clearable placeholder="退款状态" class="filter-box">
          <el-option
    ...
    <el-table :data="list" highlight-current-row border :stripe="true" v-loading="listLoading" style="width: 100%;" class="filter-box" :header-row-style="{backgroundColor: '#f6f6f6'}" @sort-change="sortTable">
      ...
      <el-table-column  align="center" label="实退金额（元）" width="120">
        <template slot-scope="scope">
      ...
      <el-table-column  align="center" label="第三方订单号" width="90">
        <template slot-scope="scope">
          <span>{{ scope.row.arrValue }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column  align="center" label="申请时间" min-width="260">
        <template slot-scope="scope">
          <span>{{ scope.row.regulations }}</span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="退款时间" min-width="260">
        <template slot-scope="scope">
          <span>{{ scope.row.regulations }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="退款状态" sortable="custom" prop="status">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status === '1'" type="success">可用</el-tag>
          <el-tag v-else type="danger">禁用</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="备注" min-width="260">
        <template slot-scope="scope">
          <span>{{ scope.row.regulations }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="180">
        <template slot-scope="scope">
          <el-button v-if="hasPermission(funcCode.couponView)" class="action" type="text" @click="applicationBond(scope.row)">退款</el-button>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      applicationBond () {
        this.$confirm('已无法原路退回，需人工处理！确认处理完成后再点击确认已退款', '提示', {
