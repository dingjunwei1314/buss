<template>
  ...
  <div class="re-re">
    ...
    <div class="table-filter">
      ...
      <div class="filter-item">
        <label for="ticket-type">支付方式：</label>
      ...
      <div class="filter-item">
        <label for="ticket-type">支付状态：</label>
        <el-select name="ticket-type" v-model="ticketType" size="small" filterable clearable placeholder="退款状态" class="filter-box">
          <el-option
            v-for="item in refundStatus"
            :key="item.number"
    ...
    <el-table :data="list" highlight-current-row border :stripe="true" v-loading="listLoading" style="width: 100%;" class="filter-box" :header-row-style="{backgroundColor: '#f6f6f6'}" @sort-change="sortTable">
      ...
      <el-table-column align="center" label="实充金额（元）" width="120" sortable="custom" prop="stype">
        <template slot-scope="scope">
      ...
      <el-table-column  align="center" label="支付状态" width="200">
        <template slot-scope="scope">
          <span v-html="scope.row.effective_type_time"></span>
        </template>
      </el-table-column>
      ...
      <el-table-column  align="center" label="时间" min-width="260">
        <template slot-scope="scope">
